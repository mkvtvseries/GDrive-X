...
function gd_cache($link, $source) {
	...
	$gdrive_source = fopen("cache/".$file_name.".txt",'w');
	fwrite($file,$string);
	fclose($gdrive_source);
...
foreach($files as $gdrive_source) { // iterate files
    // if file creation time is more than 5 minutes
    if ((time() - filectime($gdrive_source)) > 3600) {  // 86400 = 60*60*24
        unlink($gdrive_source);
    }
}
...
function getlink($id){
	...
	curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/uplay-video.mp4");
	curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . "/uplay-video.mp4");
